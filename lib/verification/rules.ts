 ...
 */
export function compareText(expected: string, found: string | null): {
  status: FieldStatus;
  note: string | null;
} {
...
function parseNumberWithFraction(text: string): number {
  ...
  if (m) {
    return (m[1] ? parseFloat(m[1]) : 0) + parseInt(m[2], 10) / parseInt(m[3], 10);
  }
...
export function compareNetContents(
  ...
  if (Math.abs(expectedMl - foundMl) < 0.5) {
    const sameText = normalizeLoose(expected) === normalizeLoose(found);
    return sameText
...
export function checkGovernmentWarning(
  ...
  if (found.toUpperCase() !== required.toUpperCase()) {
    const divergence = firstDivergence(found.toUpperCase(), required.toUpperCase());
    return {
      status: "mismatch",
...
export function runRules(
    ...
    return {
      ...
      fields: (
        [
          ...
          ["governmentWarning", "Required statutory text"],
        ] as const
      ).map(([field, expected]) => ({
        field,
        status: "not_checked",
  ...
  const classType = compareText(application.classType, extraction.classType);
  const abv = compareAbv(application.alcoholPercent, extraction.alcoholStatement);
  const net = compareNetContents(application.netContents, extraction.netContents);
