...
describe("upscaleTargetWidth — when a label image needs more pixels", () => {
  it("an image narrower than the legibility floor gets the target width", () => {
    expect(upscaleTargetWidth({ width: 900, bytes: 100_000 })).toBe(
      TARGET_WIDTH
    )
  })
  ...
  it("an image at or above the floor is left alone", () => {
    expect(
      upscaleTargetWidth({ width: MIN_LEGIBLE_WIDTH, bytes: 100_000 })
    ).toBeNull()
    expect(upscaleTargetWidth({ width: 3000, bytes: 100_000 })).toBeNull()
  })

  it("an unknown width is left alone", () => {
    expect(upscaleTargetWidth({ width: undefined, bytes: 100_000 })).toBeNull()
  })

  it("an already-huge file is left alone — the payload would balloon", () => {
    expect(
      upscaleTargetWidth({ width: 900, bytes: 8 * 1024 * 1024 })
    ).toBeNull()
  })
})
