...
export async function POST(request: Request) {
  ...
  if (!process.env.OPENAI_MODEL) {
    return NextResponse.json(
      {
        error:
          "Server is missing the OPENAI_MODEL environment variable. No default model is configured.",
      },
      { status: 500 }
    )
  }
  ...
  try {
    ...
    if (error instanceof OpenAI.APIConnectionError) {
      return NextResponse.json(
        {
          error:
            "Could not reach the AI service. Check the network and retry.",
          retryable: true,
        },
        { status: 502 }
      )
    ...
    if (error instanceof OpenAI.APIError) {
      return NextResponse.json(
        {
          error: `AI service error (${error.status}). Please retry.`,
          retryable: true,
        },
        { status: 502 }
      )
